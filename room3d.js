...
document.addEventListener('DOMContentLoaded', function() {
        ...
        // 모바일은 세로 화면이라 FOV를 넓혀서 캐릭터 전체가 보이게
        ...
        renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); // 모바일 고해상도 대응
        renderer.shadowMap.enabled = true;
        renderer.domElement.style.display = 'block'; // 캔버스 하단 여백 제거
            ...
            onInteract: function() {
                if (!isSitting) {
                    ...
                    if (distanceToFront > 0.5) {
                        const path = findPathAvoidingAll(character.position, this.frontPosition);
                        if (path.length > 0) {
        ...
        // 여러 오브젝트를 피하는 복합 경로 탐색
        function findPathAvoidingAll(start, goal) {
            const characterRadius = 0.3;
            const margin = 0.8; // 더 넓은 마진

            // 피해야 할 오브젝트 목록 (collisionObjects와 일치)
            const allObstacles = [
                { pos: new THREE.Vector3(0, 0, 0), width: 0.6, depth: 2.8, name: 'sofa' }, // 쇼파
                { pos: new THREE.Vector3(-1.5, 0, 8), width: 1.8, depth: 2.0, name: 'arcade' }, // 오락기
                { pos: new THREE.Vector3(-3.5, 0, -8), width: 1.5, depth: 1.8, name: 'treadmill' } // 런닝머신
            ];

            console.log(`경로 탐색: (${start.x.toFixed(2)}, ${start.z.toFixed(2)}) → (${goal.x.toFixed(2)}, ${goal.z.toFixed(2)})`);

            // 시작점이 장애물 실제 내부에 있는지 확인 (마진 없이 엄격하게)
            function isPointDeepInsideObs(point, obs) {
                const box = {
                    minX: obs.pos.x - obs.width / 2,
                    maxX: obs.pos.x + obs.width / 2,
                    minZ: obs.pos.z - obs.depth / 2,
                    maxZ: obs.pos.z + obs.depth / 2
                };
                return point.x >= box.minX && point.x <= box.maxX &&
                       point.z >= box.minZ && point.z <= box.maxZ;
            }

            // 시작점이 장애물 내부에 있을 때만 제외 (목표점은 제외하지 않음)
            const obstacles = allObstacles.filter(obs => {
                const startInside = isPointDeepInsideObs(start, obs);
                if (startInside) {
                    console.log(`장애물 제외: ${obs.name} (시작점이 내부에 있음)`);
                    return false;
                }
                return true;
            });
            ...
            // 점이 오브젝트 안에 있는지 확인
            function isInsideObstacle(point) {
                for (const obs of obstacles) {
                    const box = {
                        minX: obs.pos.x - obs.width / 2 - characterRadius,
                        maxX: obs.pos.x + obs.width / 2 + characterRadius,
                        minZ: obs.pos.z - obs.depth / 2 - characterRadius,
                        maxZ: obs.pos.z + obs.depth / 2 + characterRadius
                    };
            ...
            // 선분이 오브젝트와 교차하는지 확인
            function lineIntersectsAnyObstacle(p1, p2) {
                for (const obs of obstacles) {
                    const box = {
                        minX: obs.pos.x - obs.width / 2 - characterRadius,
                        maxX: obs.pos.x + obs.width / 2 + characterRadius,
                    ...
                    // 시작점이나 끝점이 박스 안에 있으면 건너뛰기 (이미 제외된 장애물)
                    const p1Inside = p1.x >= box.minX && p1.x <= box.maxX && p1.z >= box.minZ && p1.z <= box.maxZ;
                    const p2Inside = p2.x >= box.minX && p2.x <= box.maxX && p2.z >= box.minZ && p2.z <= box.maxZ;

                    if (p1Inside || p2Inside) {
                        continue; // 시작/끝점이 안에 있으면 이 장애물은 무시
                    }
            ...
            // 직선 경로 가능 여부 체크
            if (!lineIntersectsAnyObstacle(start, goal)) {
                console.log('직선 경로 사용 (복합)');
                return [goal];
            }

            // 모든 오브젝트의 모서리를 웨이포인트 후보로
            const waypoints = [];
            for (const obs of obstacles) {
                const corners = [
                    new THREE.Vector3(obs.pos.x + obs.width/2 + margin, 0.5, obs.pos.z + obs.depth/2 + margin),
                    new THREE.Vector3(obs.pos.x + obs.width/2 + margin, 0.5, obs.pos.z - obs.depth/2 - margin),
                    new THREE.Vector3(obs.pos.x - obs.width/2 - margin, 0.5, obs.pos.z + obs.depth/2 + margin),
                    new THREE.Vector3(obs.pos.x - obs.width/2 - margin, 0.5, obs.pos.z - obs.depth/2 - margin)
                ];
                for (const c of corners) {
                    if (!isInsideObstacle(c)) {
                        waypoints.push(c);
                    }
                }
            }

            // 가장 좋은 단일 웨이포인트 찾기 (시작→웨이포인트→목표가 둘 다 충돌 안 하는)
            let bestPath = null;
            let bestDist = Infinity;

            for (const wp of waypoints) {
                if (!lineIntersectsAnyObstacle(start, wp) && !lineIntersectsAnyObstacle(wp, goal)) {
                    const dist = start.distanceTo(wp) + wp.distanceTo(goal);
                    if (dist < bestDist) {
                        bestDist = dist;
                        bestPath = [wp, goal];
                    }
            ...
            if (bestPath) {
                console.log('단일 웨이포인트 경로 사용');
                return bestPath;
            }

            // 2개 웨이포인트 시도
            for (const wp1 of waypoints) {
                if (!lineIntersectsAnyObstacle(start, wp1)) {
                    for (const wp2 of waypoints) {
                        if (wp1 !== wp2 && !lineIntersectsAnyObstacle(wp1, wp2) && !lineIntersectsAnyObstacle(wp2, goal)) {
                            const dist = start.distanceTo(wp1) + wp1.distanceTo(wp2) + wp2.distanceTo(goal);
                            if (dist < bestDist) {
                                bestDist = dist;
                                bestPath = [wp1, wp2, goal];
                            }
                        }
                    }
                }
            ...
            if (bestPath) {
                console.log('2단계 웨이포인트 경로 사용');
                return bestPath;
            }

            // 최후의 수단: 그냥 목표로
            console.log('경로 탐색 실패, 직접 이동');
            return [goal];
        }
        ...
        // 모바일 감지 (위에서 이미 isMobileDevice로 정의됨)
        ...
        renderer.domElement.addEventListener('wheel', (e) => {
            ...
            // 최소/최대 거리 제한 (모바일은 더 가까이 가능)
            const minDist = isMobile ? 4 : 5;
            cameraDistance = Math.max(minDist, Math.min(30, cameraDistance));
        ...
        renderer.domElement.addEventListener('touchmove', (e) => {
            ...
            if (e.touches.length === 2) {
                ...
                if (lastTouchDistance > 0) {
                    const zoomDelta = (lastTouchDistance - currentDistance) * 0.02;
        ...
        renderer.domElement.addEventListener('touchend', (e) => {
            ...
            // 짧은 탭 (300ms 이하)이고 움직임 없으면 이동으로 처리
            const touchDuration = Date.now() - touchStartTime;
            if (touchDuration < 300 && !isTouchMoving && e.changedTouches.length === 1) {
                ...
                if (intersectPoint) {
                    ...
                    // 상호작용 중이면 무시
                    if (isWalkingToSofa || isWalkingToStand || isWalkingToArcade || isWalkingToTreadmill || isPlayingArcade || isRunningTreadmill) {
                        console.log('이동/상호작용 중 - 터치 무시');
                        return;
                    }

                    // 애니메이션 진행 중이면 무시
                    if (currentGsapTimeline && currentGsapTimeline.isActive()) {
                        console.log('애니메이션 진행 중 - 터치 무시');
                        return;
                    }
                    ...
                    if (isSitting) {
                        targetPosition = sofaFrontPosition.clone();
                        isMoving = true;
                        isWalkingToStand = true;
                    } else {
                        targetPosition = intersectPoint.clone();
                        targetPosition.y = 0.5;
                        isMoving = true;
                    }
                    console.log('터치로 이동:', targetPosition);
                }
            }
        ...
        // 카메라가 따라갈 타겟 (기본값: 원점, 이동 중에는 캐릭터)
        let cameraTarget = new THREE.Vector3(0, 0, 0);
        let cameraFollowCharacter = false;

        // 카메라 위치 업데이트
        function updateCameraPosition() {
            // 카메라가 캐릭터를 따라가는 경우 타겟 업데이트
            if (cameraFollowCharacter && character) {
                cameraTarget.copy(character.position);
                cameraTarget.y = 1; // 캐릭터 중심 높이
            }
        ...
        function typeBottomText(text) {
            ...
            const typeInterval = setInterval(() => {
                if (index < text.length) {
                    bottomText.textContent += text[index];
                    index++;
                } else {
                    clearInterval(typeInterval);
                    // 타이핑 완료 후 버튼 표시
                    if (bottomBtnGroup) {
                        bottomBtnGroup.classList.add('visible');
                    }
                }
            }, 80); // 80ms 간격으로 타이핑
        }
        ...
        window.closeCmdWindow = closeCmdWindow; // 전역 등록

        // Play 메뉴 관련 함수들
        function showPlayMenu() {
            const btnGroup = document.getElementById('bottomBtnGroup');
            const playSubmenu = document.getElementById('playSubmenu');
            if (btnGroup) btnGroup.style.display = 'none';
            if (playSubmenu) playSubmenu.classList.add('visible');
        }
        ...
        function hidePlayMenu() {
            const btnGroup = document.getElementById('bottomBtnGroup');
            const playSubmenu = document.getElementById('playSubmenu');
            if (playSubmenu) playSubmenu.classList.remove('visible');
            if (btnGroup) btnGroup.style.display = 'flex';
        }
        window.hidePlayMenu = hidePlayMenu;

        function playAction(type) {
            hidePlayMenu();

            // 모바일에서 카메라가 캐릭터를 따라가도록 활성화
            if (isMobile) {
                cameraFollowCharacter = true;
            }

            // 현재 진행 중인 활동 종료
            function stopCurrentActivity(callback) {
                let delayTime = 0;
                ...
                if (isRunningTreadmill) {
                    isRunningTreadmill = false;
                    stopTreadmillAnimation();
                    delayTime = 3500; // 트레드밀 내리기 애니메이션이 길어서 더 기다림
                }
            ...
            stopCurrentActivity(() => {
                switch(type) {
                    case 'exercise':
                        // 트레드밀 운동 - 핸들러의 onInteract 호출
                        if (treadmillHandler) {
                            treadmillHandler.onInteract();
                        }
                        break;
                    case 'game':
                        // 아케이드 게임 - 핸들러의 onInteract 호출
                        if (arcadeHandler) {
                            arcadeHandler.onInteract();
                        }
                        break;
                    case 'rest':
                        // 쇼파에 앉기 - 핸들러의 onInteract 호출
                        if (sofaHandler) {
                            sofaHandler.onInteract();
                        }
                        break;
                    case 'piano':
                        // 피아노 - 미구현
                        alert('피아노 기능은 아직 구현되지 않았습니다.');
                        break;
                    case 'hacking':
                        // 해킹 - 미구현
                        alert('해킹 기능은 아직 구현되지 않았습니다.');
                        break;
                }
            });
        }
        ...
        function playRemoteControlAction(callback) {
            ...
            console.log('=== 스마트워치 탭 모션 시작 ===');

            const watchTimeline = gsap.timeline({
                onComplete: () => {
                    console.log('=== 스마트워치 탭 완료 ===');
                    if (callback) callback();
                }
            });

            // 1. 왼팔을 앞으로 들어올리기 (워치 화면 보기)
            watchTimeline.to(leftArmGroup.rotation, {
                x: -Math.PI / 3, // -60도 (앞으로 들어올림)
                z: Math.PI / 6,  // 약간 바깥으로
                duration: 0.4,
                ease: "power2.out"
            });

            // 2. 오른팔을 왼쪽 워치 방향으로 이동 (탭 준비)
            watchTimeline.to(rightArmGroup.rotation, {
                x: -Math.PI / 4, // -45도 앞으로
                z: -Math.PI / 4, // 왼쪽으로
                duration: 0.3,
                ease: "power2.out"
            ...
            // 3. 오른손으로 워치 탭하기 (손가락으로 터치하는 느낌)
            watchTimeline.to(rightArmGroup.rotation, {
                x: -Math.PI / 3.5, // 조금 더 앞으로 (탭!)
                duration: 0.1,
                ease: "power2.in"
            });
        ...
        function animate() {
            ...
            if (isWalkingToSofa && !isMoving) {
                ...
                if (currentWaypointIndex < pathWaypoints.length) {
                    ...
                    currentWaypointIndex = 0;
                    isSitting = true;
                    cameraFollowCharacter = false; // 도착 후 카메라 따라가기 비활성화
        ...
        function startIntroSequence() {
            ...
            // 카메라 초기 설정 - 모바일/PC 분기
            if (isMobile) {
                // 모바일: 더 가깝게, 캐릭터 전신 + 얼굴이 잘 보이는 각도
                cameraDistance = 6;
                cameraAngle.theta = 0; // 정면
                cameraAngle.phi = Math.PI / 3; // 60도 (캐릭터 전신 + 얼굴 보임)
                console.log('[0초] 모바일 카메라 설정 - 거리:', cameraDistance, '각도:', 60);
            } else {
                // PC: 기존 설정
                cameraDistance = 8;
                cameraAngle.theta = 0; // 0도 (정면)
                cameraAngle.phi = Math.PI / 2.5; // 72도 (얼굴이 보이는 각도)
                console.log('[0초] PC 카메라 설정 - 거리:', cameraDistance);
            }
            updateCameraPosition();
